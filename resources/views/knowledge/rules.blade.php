    ...
    <section class="mx-auto w-full max-w-6xl space-y-6">
        ...
        <section class="space-y-4">
            @foreach ($rulebookSections as $sectionKey => $sectionHtml)
                <article class="knowledge-rulebook-article w-full max-w-none rounded-xl border border-stone-800 bg-neutral-900/65 p-5" id="regelwerk-{{ $sectionKey }}">
                    <div class="knowledge-content w-full max-w-none text-[#cccccc]">
                        {!! $sectionHtml !!}
                    </div>
                </article>
            @endforeach
