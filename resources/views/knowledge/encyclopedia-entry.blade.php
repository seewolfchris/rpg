...
@section('content')
    @php
        $entryMedia = $entry->relationLoaded('media')
            ? $entry->media
                ->where('collection_name', \App\Models\EncyclopediaEntry::ENTRY_MEDIA_COLLECTION)
                ->where('disk', 'public')
                ->values()
            : $entry->media()
                ->where('collection_name', \App\Models\EncyclopediaEntry::ENTRY_MEDIA_COLLECTION)
    ...
    <section class="ui-page-medium space-y-6">
        ...
        @if ($entryMedia->isNotEmpty())
            <section class="rounded-2xl border border-stone-800 bg-black/35 p-5 shadow-xl shadow-black/25">
                <h2 class="font-heading text-xl text-stone-100">Bilder, Karten &amp; Pläne</h2>
