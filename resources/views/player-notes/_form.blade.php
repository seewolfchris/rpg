...
<div class="space-y-5">
    <div>
        ...
        <input
            ...
            type="text"
            name="title"
            value="{{ old('title', $playerNote->title ?? '') }}"
            required
            maxlength="180"
            ...
            placeholder="z. B. Offene Fragen zur nächsten Szene"
        >
        @error('title')
    ...
    <div>
        <label for="body" class="mb-2 block text-xs font-semibold uppercase tracking-[0.12em] text-stone-300">Notiztext</label>
        <textarea
            id="body"
            name="body"
            rows="10"
            ...
            placeholder="Beobachtungen, Vermutungen, offene Punkte ..."
        >{{ old('body', $playerNote->body ?? '') }}</textarea>
        @error('body')
            <p class="mt-2 text-sm text-red-300">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-3">
        <div>
            ...
            >
                ...
                @foreach ($sceneOptions as $sceneOption)
                    <option value="{{ $sceneOption->id }}" @selected((string) old('scene_id', $playerNote->scene_id ?? '') === (string) $sceneOption->id)>
        ...
        <div>
            <label for="character_id" class="mb-2 block text-xs font-semibold uppercase tracking-[0.12em] text-stone-300">Charakter (optional)</label>
            <select
                id="character_id"
                name="character_id"
                class="w-full rounded-md border border-stone-600/80 bg-neutral-900/80 px-4 py-2.5 text-stone-100 outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-500/40"
            >
                ...
                @foreach ($characterOptions as $characterOption)
                    <option value="{{ $characterOption->id }}" @selected((string) old('character_id', $playerNote->character_id ?? '') === (string) $characterOption->id)>
        ...
        <div>
            ...
            <input
                ...
                min="0"
                max="999999"
                value="{{ old('sort_order', $playerNote->sort_order ?? '') }}"
                class="w-full rounded-md border border-stone-600/80 bg-neutral-900/80 px-4 py-2.5 text-stone-100 outline-none transition placeholder:text-stone-500 focus:border-amber-400 focus:ring-2 focus:ring-amber-500/40"
                placeholder="0"
            >
            @error('sort_order')
