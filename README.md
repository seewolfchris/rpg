...
Die Startwelt `Chroniken der Asche` bleibt erhalten und ist Teil eines mehrweltfähigen Produktmodells.
Lizenz: proprietär (`LICENSE`).
...
- Mit expliziten Skip-Gates (nur bei begründetem Sonderfall): `--skip-backend-tests`, `--skip-js-tests`, `--skip-e2e-tests`, `--skip-build`, `--skip-smoke`
...
- Default-Budgets für `posts.latest_by_id` (override via `PERF_*`):
...
- Betriebs-/Setup-Ergänzungen: `docs/PLESK_DEPLOYMENT_FUER_ANFAENGER.md`, `docs/GITHUB_PLESK_SETUP.md`
...
- v0.24-beta (Stability-Update 2026-03-23): Harte Service-Invarianten für Probe/Inventar (Welt + Kampagnen-Teilnahme), robuste Atomic-/Compensation-Semantik in `StorePostService` und `CreateCharacterAction`, Queue-Retry-Jobs für fehlgeschlagene Szenen-/Mention-Benachrichtigungen
...
- `npm run build` -> **grün** (Stand: 2026-04-02)
...
- Benachrichtigungen: In-App + Mail + Browser Web Push (VAPID, weltkontextfähig)
- Gamification: Punkte für freigegebene Posts
- Wissenszentrum: Übersicht, Wie-spielt-man, Regelwerk, Enzyklopädie
- Enzyklopädie-Admin: Kategorien/Einträge CRUD für GM/Admin
- Multi-Welt-Plattform:
  - Weltmodell + Admin-CRUD (`worlds`)
  - Weltkontext-Routing unter `/w/{world}/...`
  - Legacy-Redirects (`301`) für kampagnen-/szenenbezogene Altpfade ohne Weltsegment
  - Globale Wissensrouten ohne Weltsegment bleiben direkt erreichbar (`/wissen`, `/wissen/*`)
  - Weltbindung auf Kampagnen, Charaktere und Enzyklopädie-Kategorien
...
- Primärziel: immersives, asynchrones Geschichtenerzählen (PbP)
- Regelkomplexität: leichtes d100-/Prozent-System als Unterstützung, nicht als Selbstzweck
- UX-Ziel: Roman-Gefühl statt Regelbogen-Overload
- Versionspflege: Bei produktiven Änderungen `APP_VERSION` anheben, damit der Footer den aktuellen Stand zeigt.

## Produkt-Entscheidungen (März 2026)

- Plattform-Branding: **C76-RPG** (nicht mehr weltgebundenes Branding auf der Landingpage)
- Welten sind kampagnengebunden (eine Kampagne gehört genau zu einer Welt)
- Charaktere und Enzyklopädie-Kategorien sind ebenfalls weltgebunden
- Wissenszentrum bleibt zentrale Einstiegsseite, jedoch immer im Weltkontext
- Enzyklopädie wird redaktionell durch GM/Admin gepflegt (pro Welt getrennt)
...
- SQLite nur optional für einzelne lokale Test-Setups
- Tailwind CSS (Vite)
- Blade Templates
...
- Sonst können Feature-Tests mit `419` fehlschlagen (CSRF/Environment-Mismatch).
...
Optional für produktnahe DB-Pfade (lokal mit MySQL):
...
- Der Haupt-Testlauf in CI nutzt SQLite in-memory (`phpunit.xml`) für schnelle Reproduzierbarkeit.
- Zusätzlich läuft ein separater CI-Job gegen MySQL für `--group=mysql-concurrency` und `--group=mysql-critical`.
...
Performance-EXPLAIN für Welt-Hotpaths:
...
Das Skript:
- führt den Benchmark-Recheck aus
...
Optionaler Runtime-Toggle für den MySQL-Hotpath `posts.latest_by_id`:
...
- Queue akzeptiert nur aufgelöste Same-Origin-`POST`-Ziele; fremde Origins, nicht-HTTP(S)-Schemas oder nicht-`POST` werden verworfen.
...
- Der CSRF-Token bleibt transient im Re-Sign-Flow und wird nicht in Queue-Einträgen persisted.
- Bei `401`/`419`/`429` werden Queue-Einträge nicht verworfen, sondern mit Backoff (`retry_count`, `next_retry_at`) geplant.
- Bei `4xx` (außer `401`/`419`/`429`) wandern Queue-Einträge in `postDeadLetters` statt verworfen zu werden.
- Bei `5xx` (oder Netzwerkfehler `status=0`) erfolgen max. `5` Retries; danach wandert der Eintrag in `postDeadLetters`.
- Dead-Letter-Einträge speichern ein kurzes `error_summary` (z. B. `Text zu kurz` oder `Server-Fehler (500)`), das im UI vor dem Import angezeigt wird.
- Service Worker wird versionsgebunden registriert (`/sw.js?v=<APP_VERSION>-<APP_BUILD>`), Cache-Namen sind versionsbasiert.
- Relevante Service-Worker-Events für die UI:
  - `POST_SYNC_AUTH_RETRY`: Re-Signing erfolgreich, erneuter Sendeversuch startet.
  - `POST_SYNC_DEAD_LETTERED`: Queue-Eintrag wurde mit `error_summary` in Entwürfe/Fehler verschoben.
  - `POST_SYNC_RETRY_SCHEDULED`: Retry mit nächstem Zeitpunkt geplant.
  - `POST_SYNC_AUTH_REQUIRED`: Session/CSRF nicht erneuerbar, Nutzeraktion (Login) erforderlich.

## Crawler / KI-Bot Schutz

- `public/robots.txt` sperrt Crawling für alle Bots (`Disallow: /`).
...
Hinweis: Das ist Best-Effort. Vollständiger Schutz gegen Scraping erfordert zusätzliche Webserver-/WAF-Regeln.


## Rate Limiting für mutierende Routen

Folgende zentrale Limiter sind für schreibende Endpunkte aktiv:

- `writes`: **30 Requests/Minute je Nutzer/IP** für allgemeine Schreibaktionen (POST/PATCH/DELETE), z. B. Kampagnen/Szenen/Posts/Einladungen/Bookmarks/Subscriptions/Logout.
- `moderation`: **15 Requests/Minute je Nutzer/IP** für Moderationsaktionen, z. B. Post-Freigabe/Ablehnung, Pin/Unpin, GM-Bulk-Moderation und Enzyklopädie-Admin-CRUD.
- `notifications`: **20 Requests/Minute je Nutzer/IP** für mutierende Benachrichtigungs-Routen (`read`, `read-all`, Preferences-Update).
- `webpush-subscriptions`: **20 Requests/Minute je Nutzer/IP und Welt** für `/api/webpush/subscribe` und `/api/webpush/unsubscribe`.
...
- Enzyklopädie (canonical): `/w/{world}/wissen/enzyklopaedie`
- Enzyklopädie Admin (nur GM/Admin): `/w/{world}/wissen/enzyklopaedie/admin/kategorien`
- Dashboard: `/dashboard`
...
- Web Push benötigt Browser-Permission und aktiven Service Worker pro Endgerät
- Keine externe Medien-CDN-Optimierung (lokaler Speicher/Disk)

## Deployment-Notiz (Plesk)

- `.env` auf Produktiv-URL/DB setzen
- `PHP_BIN=/opt/plesk/php/8.5/bin/php` setzen (Projekt benötigt PHP `>= 8.5`)
